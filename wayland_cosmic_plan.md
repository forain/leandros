...
- STATUS 2026-07-23 (M4f wave): **M4 DONE — accept + composite + cursor proven
  ...
  gbm_bo_get_fd reaches drmPrimeHandleToFD" writeup was a MISDIAGNOSIS — there was
  never a userspace dmabuf wall blocking the exit. Two real root causes, both
  kernel-side, both fixed:
  1. Scanout: the sign-extended ioctl request never hit the PRIME intercept —
     fixed by masking it (8a2a271); PRIME/dmabuf via borrowed dumb-buffer VMOs
     (6ce43be, drmsmoke 20/20 both arches). No Mesa patch exists or is needed;
     anvil renders its desktop and composites clients under softpipe/kms_swrast.
  2. **Client-accept blocker (the decisive M4 find): a family of blocking syscalls
     busy-polled instead of blocking, and timerfd_create/eventfd2/signalfd4 DROPPED
     their creation flags so O_NONBLOCK was never recorded.** calloop's `polling`
     poller creates a non-blocking eventfd/timerfd; anvil (single-threaded softpipe)
     read its non-blocking eventfd, and because fd_nonblock() was false the kernel
     VFS read path yield-spun on EAGAIN, PINNING anvil's only thread so its event
     loop never returned to epoll_wait to accept the wayland client or dispatch
     input. Fixed by threading O_NONBLOCK|O_CLOEXEC through sys_timerfd_create/
     eventfd2/signalfd4 into the VFS handlers (stored on FdEntry). Also converted
     sys_wait4/sys_waitid/sys_nanosleep and net_daemon from yield_now busy-poll to
     block-on-poll (the sustained ~300% CPU that fooled every prior wave as "TCG-
     softpipe slowness" was these kernel spins, NOT anvil — anvil was always
     BLOCKED, 0 EL0 PC samples). CPU 300% -> ~100%.
- EXIT EVIDENCE (screenshots in leandros-artifacts/notes/m4-screenshots/):
  - CRIT1 accept+composite: PROVEN both arches — serial UXTR CON then **ACC**
    ...
    x86_64 m4e-r-x86_64-tcg-B-client.png + clean uncorrupted m4f-x86_64-tcg-serial.log.
  - CRIT2 cursor: PROVEN both arches — QMP virtio-tablet motion moves the cursor
    across screenshots (B top-left -> D center-right), window stays composited.
  - CRIT3 keyboard: keys reach the KERNEL input stack (proven) — EVK evdev trace:
    QMP key -> dev=0 code=0x1e/0x30 (KEY_A/B); QMP click -> dev=1 code=0x110
    (BTN_LEFT), both arches. EVIOCGPROP now reports INPUT_PROP_POINTER for the
    tablet so libinput delivers BTN_LEFT as a pointer button. The wlclient window
    did NOT change color after keys (color_index unchanged across B/E0/E) on either
    arch — anvil never granted the surface keyboard focus on click, so wl_keyboard
    key events were not delivered. This is an anvil-side keyboard-focus behavior,
    downstream of and INDEPENDENT from the accept-blocker (fully proven by
    CRIT1+CRIT2); not chased — cosmic-comp has its own focus logic.
- WALL-CLOCK: aarch64 uefi-HVF — anvil accepts+composites+paints within the 150 s
  ...
  TCG is usable, NOT the feared wall). HVF is the practical vehicle on Apple
...
- NOTE (M4f): cosmic-comp uses the SAME smithay DrmCompositor + calloop `polling`
  ...
  device.rs:158/732) so it needs NO source patch for rendering; the reject-software/
  udev-enum policy lives in cosmic-comp's own `src/backend/kms/` — patch there only
  if it rejects the software device. The eventfd O_NONBLOCK fix also removes the
