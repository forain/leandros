#!/usr/bin/env bash
# Build the spawnwedge regression test -- static musl, one arch per
...
echo "Building spawnwedge for $arch ($target_triple)..."
(
    cd "$HERE"
...
echo "built: $HERE/target/$target_triple/release/spawnwedge"
