...
void DG_DrawFrame()
{
    if (FrameBuffer)
    {
        if (s_ScreenWidth == DOOMGENERIC_RESX && s_ScreenHeight == DOOMGENERIC_RESY && s_PositionX == 0 && s_PositionY == 0)
        {
        ...
        else
        {
            ...
            if (x_map == NULL || last_width != s_ScreenWidth)
            {
                if (x_map) free(x_map);
                x_map = malloc(s_ScreenWidth * sizeof(int));
                if (x_map)
                {
                    for (int x = 0; x < s_ScreenWidth; x++)
                    {
                        x_map[x] = x * DOOMGENERIC_RESX / s_ScreenWidth;
                    }
                }
                last_width = s_ScreenWidth;
            }

            if (x_map)
            {
                int last_src_y = -1;
                uint32_t* cached_src_row = NULL;

                for (int y = 0; y < s_ScreenHeight; y++)
                {
                    int src_y = y * DOOMGENERIC_RESY / s_ScreenHeight;
                    uint32_t* dst_row = (uint32_t*)FrameBuffer + y * s_ScreenWidth;

                    if (src_y != last_src_y)
                    {
                        cached_src_row = (uint32_t*)DG_ScreenBuffer + src_y * DOOMGENERIC_RESX;
                        last_src_y = src_y;
                    }

                    for (int x = 0; x < s_ScreenWidth; x++)
                    {
                        dst_row[x] = cached_src_row[x_map[x]];
                    }
                }
            }
        }
