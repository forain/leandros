#!/usr/bin/env python3
# M7k: per-TID wedge capture. Launch busd TRACED (m7repro armexec arms the kernel
# ring for busd's tgid, then execve's busd), let cosmic-comp connect + wedge at
# peer:created, then dump the ring (R7| lines) 10+s into the wedge.
...
CSET = int(sys.argv[4]) if len(sys.argv)>4 else 45   # host sleep after launch
ENV = [
...
def main():
    log(f"==== M7k capture {ARCH} {MODE} {TAG} {time.ctime()} ====")
    booted=False
    ...
    log(f"[host sleep {CSET}s for comp->busd wedge]"); time.sleep(CSET)
    log("=== busd.log (peer created?) ==="); dump("cat /tmp/busd.log 2>&1", t=40, keep=8000)
    log("=== comp.log tail ==="); dump("tail -15 /tmp/comp.log 2>&1", t=25, keep=2500)
    marker=f"M7K-DUMP-{TAG}"; d("cmd",f"echo {marker}",t=6)
