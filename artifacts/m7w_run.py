#!/usr/bin/env python3
# M7w milestone run: boot, login, launch the full COSMIC session with a
# persistent serial drainer, screenshot the framebuffer at several settle
# points. Goal: the panel bar renders now that leandros-applet gives the panel
# real content (forces actual_size>20 -> render() commits frame 0).
import subprocess, sys, os, time, threading, re
DRIVER = os.path.expanduser("~/code/leandros/.claude/skills/run-leandros/driver.py")
SERIAL_LOG = "/tmp/leandros-serial.log"
OUT = os.path.expanduser("~/code/leandros-artifacts/notes/m7w-screenshots")
ARCH = sys.argv[1] if len(sys.argv) > 1 else "aarch64"
MODE = sys.argv[2] if len(sys.argv) > 2 else "uefi"
TAG  = sys.argv[3] if len(sys.argv) > 3 else "m0"
DRAIN = int(sys.argv[4]) if len(sys.argv) > 4 else 175
SHOTS = [45, 70, 95, 120, 150]
...
def main():
    log(f"==== M7w run {ARCH} {MODE} tag={TAG} {time.ctime()} ====")
    ...
    for when in SHOTS:
        ...
        ppm = f"{OUT}/m7w-{ARCH}-{TAG}-t{when}.ppm"
    ...
    try:
        ...
        open(f"{OUT}/m7w-{ARCH}-{TAG}-serial.txt", "w").write(clean_txt[-800000:])
        for key in ("leandros-applet", "Starting:", "cosmic-panel", "GL Renderer",
                    "softpipe", "far=", "EL0 Fault", "panic", "Unknown id",
                    "Broken pipe", "PANEL MAIN ERR", "Initializing OpenGL",
                    "com.system76.CosmicAppletTime", "Rendering space"):
