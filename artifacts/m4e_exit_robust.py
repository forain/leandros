...
CAP = f"{OUT}/m4e-robust-{TAG}-serial.log"
...
def main():
    ...
    log("...persistent reader attached; settling 150s...")
    time.sleep(150)
    shot("B-client")
    ...
    qmp("move", str(tx), str(ty)); time.sleep(1)   # pointer over the composited client window
    qmp("click"); time.sleep(1)                     # click -> anvil sets keyboard focus to that surface
    shot("E0-focusclick")
    qmp("key","a"); time.sleep(1); qmp("key","b"); time.sleep(3)
    shot("E-key")
    _stop.set(); th.join(timeout=5)
    # summarize
    txt = open(CAP, errors='replace').read()
    hits = [l for l in txt.splitlines() if any(k in l for k in ("UXTR","wlclient","M4RUN","KEY code"))]
    log(f"--- serial evidence ({len(hits)} lines, cap={len(txt)}B) ---")
    for l in hits[-50:]: log("  "+l.strip()[:170])
