#!/usr/bin/env python3
# M7k: desktop-free W1 repro + per-TID capture. Launch busd TRACED (armexec),
# then a COALESCING client (m7repro coalclient: connect + write AUTH+Hello in one
# send, comp-style) against busd. Capture the ring at the (possible) wedge.
...
TAG  = sys.argv[3] if len(sys.argv)>3 else "coal0"
CWAIT= int(sys.argv[4]) if len(sys.argv)>4 else 20
NPIPE= sys.argv[5] if len(sys.argv)>5 else "0"
SCRIPT = [
    "mkdir -p /run/user/0",
    "rm -f /run/user/0/bus",
    ...
    "sleep 5",
    "echo CLIENT_LAUNCH",
    f"/bin/m7repro coalclient /run/user/0/bus {NPIPE} >/tmp/cli.log 2>&1 &",
...
def main():
    log(f"==== M7k coalcap {ARCH} {MODE} {TAG} {time.ctime()} ====")
    booted=False
    ...
    log(f"[host sleep {CWAIT}s for coalesced client -> wedge]"); time.sleep(CWAIT)
    log("=== cli.log (HELLO_ANSWERED or wedge?) ==="); dump("cat /tmp/cli.log 2>&1", t=25, keep=2000)
    log("=== busd.log (peer created?) ==="); dump("cat /tmp/busd.log 2>&1", t=40, keep=9000)
    marker=f"M7K-COAL-{TAG}"; d("cmd",f"echo {marker}",t=6)
    ...
    try:
        ...
        idx=data.rfind(marker); window=data[idx:] if idx>=0 else data[-200000:]
        dst=f"{OUT}/m7k-coal-{ARCH}-{TAG}.log"; open(dst,"w").write(window)
        nR=window.count("R7| t="); log(f"[ring window->{dst} {len(window)}B {nR} recs]")
    except Exception as e: log(f"[err]{e}")
    clean(); log("==== coalcap DONE ====")
