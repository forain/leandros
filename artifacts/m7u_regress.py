#!/usr/bin/env python3
# M7u regression: fresh boot, login root, run vfstest FIRST, then scmtest (with
...
def main():
    log(f"==== M7u regress {ARCH} {MODE} tag={TAG} {time.ctime()} ====")
    booted = False
    ...
    for cmd, dur in TESTS:
        ...
        open(f"{OUT}/m7u-{TAG}-{ARCH}-{cmd}.txt", "w").write(clean_txt)
