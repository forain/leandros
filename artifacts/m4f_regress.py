...
def main():
    ...
    log(f"==== M4f REGRESSION {ARCH} {MODE} {time.ctime()} ====")
    ...
    d("screenshot", f"{OUT}/m4f-kmscube-{ARCH}-1.ppm", t=30)
    dcmd("sleep 1.5", t=6)
    d("screenshot", f"{OUT}/m4f-kmscube-{ARCH}-2.ppm", t=30)
    try:
        a=open(f"{OUT}/m4f-kmscube-{ARCH}-1.ppm",'rb').read()
        b=open(f"{OUT}/m4f-kmscube-{ARCH}-2.ppm",'rb').read()
