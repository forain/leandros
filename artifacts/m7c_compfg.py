#!/usr/bin/env python3
# M7c: run cosmic-comp with output UNBUFFERED to the console (no file redirect,
# so stderr is line-buffered to the tty) to see EXACTLY where it stops. The
# file-redirect ">/tmp/comp.log 2>&1" used by m6h/m7b block-buffers stderr, so
# an early hang leaves the file empty and masks whether comp launched at all.
...
TAG  = sys.argv[3] if len(sys.argv)>3 else "fg"
...
def main():
    log(f"==== M7c compfg {ARCH} {MODE} {TAG} {time.ctime()} ====")
    booted=False
    ...
    # comp in FOREGROUND, unbuffered to console. RUST_LOG=info for its own logs.
    d("cmd","export RUST_LOG=info","8")
    log(f"[run cosmic-comp FOREGROUND, capture {CWAIT}s of streamed output]")
    out = d("cmd","cosmic-comp --no-xwayland 2>&1", t=CWAIT)
    log("=== COMP FOREGROUND STREAM ===")
    log(deansi(out)[-8000:])
    # if we get here comp either exited or driver timed out (still running).
    # Probe liveness + busd peer state from a fresh cmd (send Ctrl-C first via stop? no—
    # comp is foreground; the driver timeout returns but comp still owns the tty. Use monitor.)
    log("=== busd.log (peer created?) ===")
    # busd log is independent; read serial file directly for busd lines
    try:
        data=open(SERIAL_LOG,errors="replace").read()
        open(f"{OUT}/m7c-compfg-{ARCH}-{TAG}-serial.log","w").write(data[-200000:])
        log(f"[serial saved {len(data)}B]")
    except Exception as e: log(f"[serr]{e}")
    clean(); log("==== compfg DONE ====")
