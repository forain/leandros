#!/bin/sh
# M4 closure verifier: resolves anvil's full transitive DT_NEEDED against the
# UNION of {m4-input-ship/<arch>/usr/lib (M4 staged set, libudev flipped)} and
# {m3-gl-stack/sysroot-<arch>/usr/lib (M3 base+GL set)}, with M4 taking
# precedence (it should shadow the sysroot's stock libudev.so.1).
# Read-only against both source trees. Usage: verify-closure.sh <arch> <elf>
set -e
