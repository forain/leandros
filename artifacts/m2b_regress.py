#!/usr/bin/env python3
# M2b regression: fresh boot, login root, run vfstest FIRST, then scmtest (with
...
def main():
    log(f"==== M2b regress {ARCH} {MODE} tag={TAG} {time.ctime()} ====")
    booted = False
    ...
    for cmd, dur in TESTS:
        ...
        open(f"{OUT}/m2b-{TAG}-{ARCH}-{cmd}.txt", "w").write(clean_txt)
