...
// Kernel-mode synchronous exceptions go straight to exc_el1_sync.
//
// Until 2026-09-25 they first went through a diagnostic capture stub
// (exc_el1_sync_capture, from the HVF fork-race investigation) that stashed
// x1 in PAR_EL1 while it recorded ESR/ELR/FAR. PAR_EL1 is not a full 64-bit
// scratch register on real hardware: with F=0 several fields are RES0/RES1,
// so the read-back returned a different x1. Under TCG the round trip was
// exact, which hid it; under HVF every recoverable EL1 fault (a kernel store
// into a demand-paged or copy-on-write user page) resumed the interrupted
// kernel code with a corrupted x1 — observed as a unix-socket recv that
// reported 64 bytes and stored them nowhere the caller could see.
